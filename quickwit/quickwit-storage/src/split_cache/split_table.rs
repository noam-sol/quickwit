...
impl SplitTable {
    ...
    /// download list.
    pub fn touch(&mut self, split_ulid: Ulid, storage_uri: &Uri, index_id: String) -> Option<u64> {
    ...
    pub(crate) fn report(&mut self, split_ulid: Ulid, storage_uri: Uri, index_id: String) {
...
mod tests {
    ...
    fn test_split_table_prefer_last_touched() {
        ...
        let num_bytes_opt = split_table.touch(
            ulid1,
            &Uri::for_test("s3://test1/"),
            "test-index".to_string(),
        );
    ...
    fn test_report_with_index_id() {
        ...
        let split_ulid = Ulid::new();
        let index_id = "test-index-id".to_string();

        // Report with index_id
        split_table.report(
            split_ulid,
            Uri::for_test(TEST_STORAGE_URI),
            index_id.clone(),
        ...
        // Check if the index_id is properly stored in the split info
        let candidate = split_table.start_download(split_ulid).unwrap();
        assert_eq!(candidate.split_ulid, split_ulid);
        assert_eq!(candidate.index_id, index_id);

        // We've removed the split when we started downloading it, so we need to report it again
        split_table.report(
            split_ulid,
            Uri::for_test(TEST_STORAGE_URI),
            index_id.clone(),
        ...
        // We can access the split_to_status directly in tests
        // No need to check split_info.index_id as we've removed it
    }
}
